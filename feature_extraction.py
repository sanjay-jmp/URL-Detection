...
output_csv_filename = 'feature_selected.csv'
data_to_write=[]
with open(csv_filename, 'r') as csv_file:
    ...
    for row in csv_reader:
        if row:  # Check if the row is not empty
            ...
            i+=1
            print(i)
            other_values=generate_dataset(first_column_value)
            data_to_write.append([first_column_value]+other_values)
            with open(output_csv_filename, 'w', newline='') as output_csv_file:
                csv_writer = csv.writer(output_csv_file)
                # Write the header for the output CSV
                csv_writer.writerow(
                    ['First Column', 'having ip address', 'url length', 'shortening services', 'having @ symbol',
                     'double slash', 'prefix suffix', 'having sub domain', 'ssl final state', 'domain reg', 'favicon',
                     'port', 'https token', 'request url', 'url anchor', 'links in tag', 'sfh', 'submitting to email',
                     'abnormal url', 'redirect', 'on mouse over', 'right click', 'popup window', 'iframe',
                     'age of domain', 'dns record', 'google index', 'links pointing to page', 'statistical report'])
                # Write the data
...
with open(output_csv_filename, 'w', newline='') as output_csv_file:
    csv_writer = csv.writer(output_csv_file)
    # Write the header for the output CSV
    csv_writer.writerow(['First Column', 'having ip address','url length','shortening services','having @ symbol','double slash','prefix suffix','having sub domain','ssl final state','domain reg','favicon','port','https token','request url','url anchor','links in tag','sfh','submitting to email','abnormal url','redirect','on mouse over','right click','popup window','iframe','age of domain','dns record','google index','links pointing to page','statistical report'])
    # Write the data
    csv_writer.writerows(data_to_write)

def assess_risk(url):
    # Generate features for the given URL
    features = generate_dataset(url)

    # Sum the feature values to assess risk
    risk_score = sum(features)

    # Interpret the risk score and print the result
    print("URL:", url)
    if risk_score > 0:
...
if __name__ == "__main__":
    # Example URL to test (replace with the URL you want to test)
    url_to_test = "http://example.com"

    # Test the URL
    assess_risk(url_to_test)